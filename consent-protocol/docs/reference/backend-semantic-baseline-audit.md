...
| Domain registry | `domain_registry_service.py` | Legacy compatibility registry only | N/A | Yes | `mixed_transitional` | Keep for compatibility only; do not use as runtime semantic source for PKM classification or scope derivation | phase 1 cleanup |
...
| Legacy `world_model.*` compatibility routes and aliases | `api/routes/world_model.py`, consent/mcp legacy aliases | Compatibility and legacy naming still visible | No | Partly | `drift_or_legacy` | Keep compatibility bounded, but stop using it to explain or shape PKM semantics | phase 1 cleanup |
...
- `world_model.*` compatibility remains bounded compatibility, not future architecture.
